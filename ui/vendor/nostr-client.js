...
// src/nostr/event-builder.js
var NIP53_LIVE_EVENT_KIND = 30311;
var ZAPCAST_CLIENT_TAG = "zapcast";
var ZAPCAST_DISCOVERY_URL = "https://zapcast.live";
function buildZapcastLiveEvent({ streamId, title, description, status = "live", createdAt = unixNow() } = {}) {
...
function buildZapcastEndedEvent({ streamId, title = "zapcast live stream", description = "", createdAt = unixNow() } = {}) {
...
// src/nostr/parser.js
var LIVE_STREAM_MAX_AGE_SECONDS = 24 * 60 * 60;
function tagValue(event, name) {
  const tag = event?.tags?.find((tag2) => Array.isArray(tag2) && tag2[0] === name && tag2[1]);
  return tag ? String(tag[1]) : "";
}
function tagValues(event, name) {
  return (event?.tags || []).filter((tag) => Array.isArray(tag) && tag[0] === name && tag[1]).map((tag) => String(tag[1]));
}
function hasTagValue(event, name, value) {
  return tagValues(event, name).some((item) => item.toLowerCase() === String(value).toLowerCase());
}
function extractZapcastStreamId(event) {
...
function parseZapcastLiveEvent(event, { now: now2 = unixNow2(), maxAgeSeconds = LIVE_STREAM_MAX_AGE_SECONDS } = {}) {
  ...
  if (maxAgeSeconds > 0 && now2 - createdAt > maxAgeSeconds) return invalid("stream announcement is stale");
...
function filterLiveZapcastEvents(events = [], options = {}) {
...
// src/nostr/relays.js
var DEFAULT_NOSTR_RELAYS = Object.freeze([
  "wss://relay.damus.io",
  "wss://nos.lol",
...
function parseRelayList(value, { fallback = DEFAULT_NOSTR_RELAYS } = {}) {
...
function nostrDiscoveryFilter({ limit = 50 } = {}) {
  return {
    kinds: [30311],
    "#t": ["zapcast"],
    limit
...
function identityFromSecret(value) {
  const secret = decodeSecret(value);
  const pubkey = getPublicKey(secret);
  return encodeIdentity(secret, pubkey);
}
async function publishLiveStream({ identity, relays, streamId, title, description }) {
  return publishEvent({
    identity,
    relays,
...
async function publishEndedStream({ identity, relays, streamId, title, description }) {
  return publishEvent({
    identity,
    relays,
...
async function discoverLiveStreams({ relays, limit = 50, now: now2 } = {}) {
  const normalizedRelays = parseRelayList(relays);
  if (!normalizedRelays.length) throw new Error("No Nostr relays configured.");
  const pool = new SimplePool();
  try {
    const events = await pool.querySync(normalizedRelays, nostrDiscoveryFilter({ limit }), { maxWait: 5e3 });
    const streams = filterLiveZapcastEvents(events, { now: now2 }).map((stream) => ({
      ...stream,
      npub: stream.pubkey ? nip19_exports.npubEncode(stream.pubkey) : ""
    }));
...
async function publishEvent({ identity, relays, template }) {
  ...
  const pool = new SimplePool();
  try {
    const results = await Promise.allSettled(pool.publish(normalizedRelays, event, { maxWait: 5e3 }));
    const successes = [];
    const failures = [];
...
function encodeIdentity(secret, pubkey) {
  const secretKeyHex = bytesToHex3(secret);
  return {
    secretKeyHex,
    pubkey,
    npub: nip19_exports.npubEncode(pubkey),
    nsec: nip19_exports.nsecEncode(secret)
  };
}
function decodeSecret(value) {
  ...
  if (raw.startsWith("nsec")) {
    const decoded = nip19_exports.decode(raw);
